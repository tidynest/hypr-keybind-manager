...
//! - **Normal text**: 2.0-3.0 bits/char (predictable patterns, common letters)
//! - **Base64**: 2.5-4.5 bits/char (varies with source data and length)
//! - **Hex**: 2.0-4.0 bits/char (theoretical max 4.0, actual lower due to patterns)
//! - **Detection**: Pattern matching (alphabet, length) + entropy validation
//!
//! **Note on entropy:** Theoretical maximums (base64=6.0, hex=4.0) are rarely achieved
//! in practice due to repetition, padding, and short string length. We use realistic
//! thresholds based on actual encoded strings.
//!
...
impl DangerDetector {
    ...
    /// - Base64: ~2.5-4.5 bits/char (varies with data, padding lowers entropy)
    /// - Hex: ~2.0-4.0 bits/char (theoretical max 4.0, actual lower with patterns)
    /// - Random binary: ~8.0 bits/char (theoretical maximum, rarely seen)
    ///
    /// **Reality vs Theory:** While base64 theoretical max is 6.0 and hex is 4.0,
    /// actual strings score lower due to repetition, padding, and short length.
    ///
    /// # Examples
    /// ```
    ...
    ///
    /// Base64 has very specific structural properties:
    /// - Character set: A-Z, a-z, 0-9, +, /, =
    /// - Length divisible by 4 (padding ensures this)
    /// - Often ends with = or == padding
    ///
    /// We prioritise structural checks over entropy because:
    /// - Short base64 strings have variable entropy (2.5-4.0 bits/char)
    /// - Source data patterns affect encoded entropy
    /// - Padding and length are more reliable signals
    ...
    /// Detects if a string is likely hex encoded
    ///
    /// Hex characteristics:
    /// - Medium-high entropy (~4.0 bits/char)
    /// - Character set: 0-9, a-f, A-F only
    /// - Even length (2 hex chars = 1 byte)
